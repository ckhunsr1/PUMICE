...
elnet <- function(geno_tuning, expression_tuning, geno_testing, expression_testing, penalty_k, lambda_list) {
        ...
        testing <- tryCatch({
                alpha.fit <- glmnet(t(geno_tuning), t(expression_tuning), alpha = 0.5, family="gaussian", penalty.factor = penalty_k[rownames(geno_tuning)], standardize = F, lambda = lambda_list)

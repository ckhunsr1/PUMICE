...
glm_cv <- function(geno_training, expression_training, geno_validating, expression_validating, geno_tuning, expression_tuning, geno_testing, expression_testing) {
...
expression = expression %>% select(gene_id, sample_overlap) %>% remove_rownames %>% column_to_rownames(var="gene_id")

#########################################################################################################################
                                        ##Import DAPG result##
#########################################################################################################################
##Import prior file from DAPG##
load(paste("/gpfs/group/dxl46/default/private/poom/dapg/prior/", tissue, "/prior_chr", chr_num, ".RData", sep = "")) ##variable final_df

##Filter for gene with at least one snp with pip > 0.01 and only include snp with pip > 0.01##
final_df_1 = final_df %>% filter(pip > 0.01)

##For genes without any snp with pip > 0.01, we keep all snps##
final_df_2 = final_df[!(final_df$gene %in% final_df_1$gene), ]

final_df = rbind(final_df_1, final_df_2)

##Create map file for every model whether there is any pip > 0.01 or not##
map = final_df %>% group_by(gene) %>% mutate(snp = paste0(snp, collapse = ";")) %>% select(gene, snp) %>% distinct(gene, .keep_all =TRUE)
...
for (i in 1:nrow(map)) {
	...
	geno_subset = geno[unlist(strsplit(as.character(map[i,2]), ";")),]
	expression_subset = expression[as.character(map[i,1]),]
	origin_id = NA
	...
	if (nrow(geno_subset) > 1) {
		...
		for (k in 1:1){
                	...
                	enc_imp = rep(0,fold)
			penalty_temp = final_df %>% filter(gene == as.character(map[i,1]))
			penalty_temp = penalty_temp[penalty_temp$snp %in% rownames(geno_subset), ]
			penalty_k = penalty_temp[match(rownames(geno_subset), penalty_temp$snp), "pf"]
			...
			prediction_validating = prediction_validating[match( colnames(expression_subset), rownames(prediction_validating) ),]
                	...
                	els_output = data.frame("gene_id" = rownames(expression_subset), "type" = 1000, "region" = origin_id,
                                                "penalty" = "dapg", "cvm_v" = cvm_validating, "pear_avg_v" = pear_avg_v, "spear_avg_v" = spear_avg_v,
			...
			filename_iv_result <- paste(dir_path, "/dapg/internal_validation/", tissue, "/", tissue, "_chr", chr_num, "_GTEx_dapg_internal_validation_", total_file_num, ".", file_num, ".txt", sep="")
		...
		penalty_temp = final_df %>% filter(gene == as.character(map[i,1]))
                penalty_temp = penalty_temp[penalty_temp$snp %in% rownames(geno_subset_all), ]
                penalty_k = penalty_temp[match(rownames(geno_subset_all), penalty_temp$snp), "pf"]		
		...
		if ( all(alpha.fit$beta == 0) ) {
			summary_temp = data.frame( "tissue" = tissue, "gene_id" = rownames(expression_subset_all), 
					   "func_type" = 1000, "region" = origin_id, fit = "elnet", 
					   "alpha" = 0.5, "penalty" = "dapg",
		...
		filename_summ_result <- paste(dir_path, "/dapg/summary/", tissue, "/", tissue, "_chr", chr_num, "_GTEx_dapg_summary_", total_file_num, ".", file_num, ".txt", sep="")
		write.table(summary, filename_summ_result, quote=FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)

		filename_coef_result <- paste(dir_path, "/dapg/output/", tissue, "/", tissue, "_chr", chr_num, "_GTEx_dapg_coef_", total_file_num, ".", file_num, ".txt", sep="")
		...
		for (f in 1:fold) {
   			print(f)
                        ...
                        result = glm_cv(geno_training, expression_training, geno_validating, expression_validating, geno_tuning, expression_tuning, geno_testing, expression_testing)
		...
		##Fitting glm to all data
	        x = as.data.frame(t(geno_subset_all))
	        colnames(x)[1] = "x"
	        y = as.data.frame(t(expression_subset_all))
	        colnames(y)[1] = "y"
	        df = as.data.frame(cbind(x, y))
	        glm.fit = glm(y ~ x, data = df)

		if ( glm.fit$coefficients[2] == 0 ) {
                        summary_temp = data.frame( "tissue" = tissue, "gene_id" = rownames(expression_subset_all),
                                           "func_type" = 1000, "region" = origin_id, fit = "glm",
                                           "alpha" = "NA", "penalty" = "dapg",
                                           "input_snps" = nrow(geno_subset_all), "nonzero_snps" = 0)
                        summary = rbind(summary, summary_temp)
                } else {
                        coef_output = data.frame( "tissue" = tissue, "gene_id" = rownames(expression_subset_all),
                                          "snp" = rownames(geno_subset_all), "weight" = glm.fit$coefficients[2]) %>% remove_rownames
                        coef = rbind(coef, coef_output)

                        summary_temp = data.frame( "tissue" = tissue, "gene_id" = rownames(expression_subset_all),
                                                   "func_type" = "1000", "region" = origin_id, fit = "glm",
                                                   "alpha" = "NA", "penalty" = "dapg",
                                                   "input_snps" = nrow(geno_subset_all), "nonzero_snps" = 1)
                        summary = rbind(summary, summary_temp)
                }
                }, error=function(e){})
...
dummy="I am done"
filename_dummy = paste(dir_path, "/dapg/dummy/", tissue, "/dummy_testing_chr", chr_num, "_", total_file_num, ".", file_num, "_new.txt", sep="" )

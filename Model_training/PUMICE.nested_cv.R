...
elnet <- function(geno_training, expression_training, fold_training, geno_validating, expression_validating, geno_tuning, expression_tuning, fold_tuning, geno_testing, expression_testing, penalty_k) {
	...
	##Fit best lambda in training set and predict in validating sample
		...
		alpha.fit <- cv.glmnet(t(geno_training), t(expression_training), alpha = 0.5, family="gaussian", penalty.factor = penalty_k[rownames(geno_training)], standardize = F, nfolds = opt$fold, type.measure='mse', foldid = fold_training)
        ...
        testing <- tryCatch({
                alpha.fit <- cv.glmnet(t(geno_tuning), t(expression_tuning), alpha = 0.5, family="gaussian", penalty.factor = penalty_k[rownames(geno_tuning)], standardize = F, nfolds = opt$fold, type.measure='mse', foldid = fold_tuning)

...
opt$tmp = "/gpfs/group/dxl46/default/private/poom/DPR/temp_MCMC"
opt$noclean = FALSE
...
weights.dpr = function( input , es, snp , out=NA ) {
	...
	tryCatch({	
        arg = paste( opt$PATH_dpr , " -bfile " , input , " -dpr 2 -w 10000 -s 10000 -outdir ", opt$tmp, " -o " , out , sep='' )
...
for (i in 1:nrow(pheno_original)){
	...
	N.tot = nrow(genos$bed)
		...
		summ_fixed_temp = data.frame("tissue" = tissue, "gene_id" = rownames(pheno_original)[i], "func_type" = 1000, "region" = map[i, 2],
						"fit" = "mcmc_fixed", "input_snps" = nrow(genos$bim), "nonzero_snps" = nrow(coef_nz_fixed))
		...
		coef_nz_add = as.data.frame(wgt.matrix[which(wgt.matrix[,2] != 0), 2])
                summ_add_temp = data.frame("tissue" = tissue, "gene_id" = rownames(pheno_original)[i], "func_type" = 1000, "region" = map[i, 2],
                                                "fit" = "mcmc_add", "input_snps" = nrow(genos$bim), "nonzero_snps" = nrow(coef_nz_add))
		...
		filename = paste("/gpfs/group/dxl46/default/private/poom/GTEx/coef_total/mcmc_fixed/internal_validation/", tissue, "/", tissue,  "_chr", chr_num, 
				"_GTEx_mcmc_fixed_internal_validation_", total_file_num, ".", file_num, ".txt", sep = "")
		write.table(iv_fixed, filename, quote=FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)

		filename = paste("/gpfs/group/dxl46/default/private/poom/GTEx/coef_total/mcmc_fixed/summary/", tissue, "/", tissue,  "_chr", chr_num, 
				"_GTEx_mcmc_fixed_summary_", total_file_num, ".", file_num, ".txt", sep = "")
	        write.table(summ_fixed, filename, quote=FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)

		filename = paste("/gpfs/group/dxl46/default/private/poom/GTEx/coef_total/mcmc_fixed/output/", tissue, "/", tissue,  "_chr", chr_num, 
				"_GTEx_mcmc_fixed_coef", total_file_num, ".", file_num, ".txt", sep = "")
		write.table(coef_fixed, filename, quote=FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)

		##Write output for additive##
		filename = paste("/gpfs/group/dxl46/default/private/poom/GTEx/coef_total/mcmc_add/internal_validation/", tissue, "/", tissue,  "_chr", chr_num,
	                        "_GTEx_mcmc_add_internal_validation_", total_file_num, ".", file_num, ".txt", sep = "")
	        write.table(iv_add, filename, quote=FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)

	        filename = paste("/gpfs/group/dxl46/default/private/poom/GTEx/coef_total/mcmc_add/summary/", tissue, "/", tissue,  "_chr", chr_num,
	                        "_GTEx_mcmc_add_summary_", total_file_num, ".", file_num, ".txt", sep = "")
	        write.table(summ_add, filename, quote=FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)

	        filename = paste("/gpfs/group/dxl46/default/private/poom/GTEx/coef_total/mcmc_add/output/", tissue, "/", tissue,  "_chr", chr_num,
	                        "_GTEx_mcmc_add_coef", total_file_num, ".", file_num, ".txt", sep = "")
...
dummy="I am done"
filename = paste("/gpfs/group/dxl46/default/private/poom/GTEx/coef_total/mcmc_fixed/dummy/", tissue, "/dummy_testing_chr", chr_num,
